...
- 一次情報の検証:
  - 各トピックに必ず一次情報URL（公式ドキュメント/GitHub/リリースノート）を付ける。
  - 「npmに存在する」「GitHubにリポジトリがある」だけでは不十分。README・ドキュメントが充実し、実態のあるプロジェクトのみ採用する。
  - X上で1-2人しか言及していない、かつ公式ドキュメントが見つからないものは除外する。
- カテゴリごとに該当トピックが${input.topN}個に満たない場合:
  - 無理に埋めず「該当なし（直近${input.hours}時間で技術的に重要な動きなし）」と明記する。
  - 品質を水増しで下げるより、空欄のほうが有益。
...
2) 各領域から技術的に重要なトピックを最大${input.topN}つ抽出する（選定基準に従う）
3) 各トピックについて「エンジニアが知っておくべきこと」と「試せること」を整理する
4) 一次情報URLの実在性を再確認する（URLが見つからないトピックは削除する）
5) 領域横断で「今日のキャッチアップサマリー」をまとめる

出力形式（Markdown）:

## キャッチアップサマリー
（今日エンジニアが押さえておくべきポイントを3〜5行で。技術的な要点のみ。該当トピックが少ない場合は「静かな日」と正直に書く。）
...
- 一次情報: [公式ドキュメント/GitHub/リリースノート等のURL]（実在確認済み）
- 参考ポスト: [ポストの要旨] — URL（あれば）

（トピックが${input.topN}個に満たない場合は「該当なし」と明記）

（以下、全カテゴリ同じ構造で繰り返す）

## 深掘り候補
（技術記事・投稿ネタとして特に有益なテーマを最大3つ。以下の条件をすべて満たすもののみ:
- 一次情報（公式ドキュメント/GitHub）が充実している
- deep-research で掘っても中身がある（表面的な話題でない）
