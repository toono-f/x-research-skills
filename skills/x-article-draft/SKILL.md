...
4. **記事構成の設計**
   ...
   **導入（200〜400字）**
   - フック: 読者の関心を引く1〜2文
   - 背景: なぜ今このテーマか
   - 前提知識レベルを明示する（例: 「Next.js でプロダクション運用の経験がある方向けです」）

   **本論（2000〜4000字）**
   - 見出しは3〜5個
   - 各セクションにファクト（出典付き）を含める
   - コードブロックがある場合は本番想定の例を示す（エラーハンドリング・型定義を含む）
   - トレードオフセクションを必ず含める（メリット/デメリットの両面、代替手段との比較）
   - 定量データ（ベンチマーク、コスト、レイテンシ等）を可能な限り含める

   **まとめ（200〜400字）**
   - 要点を3行以内で
   - 採用すべきか/すべきでないかの判断基準を明確にする
   - どのケースで有効で、どのケースで見送るべきかを書く
...
6. **文体チェック**
   - `writing-style.md` に照らして以下を確認:
     - 一人称が「わたし」に統一されているか（「私」「筆者」「自分」はNG）
     - 文末が「です/ます」基調 + 口語混じりになっているか（「である」調はNG）
     - 導入が共感 or 体験から入っているか（堅い前置きはNG）
     - 適度に絵文字が入っているか（要所で使う、多すぎない）
     - 読者への呼びかけが押し付けでないか（「ぜひ試してみてください」系）
     - 硬い表現がないか（「本記事」「筆者」「〜と考えられる」「〜すべき」はNG）
   - 違反があれば修正してから出力する
...
- [ ] です/ます基調（である調なし）
- [ ] 絵文字は記事全体で3〜5個以内
- [ ] ポップすぎる表現なし（爆速/神/やばい/🔥連打等）
- [ ] 硬い表現なし（本記事/筆者/〜すべき等）
- [ ] AI生成感のある定型パターンなし（「この記事でわかること」等）
- [ ] 外部URLは適度にカード表示（直貼り）を使っている
...
- コード: ある場合は本番想定の例（エラーハンドリング・型定義含む、コピペで試せる）
- 出典: ファクトには必ずURLまたは「As of」付き
- 文体: `writing-style.md` に準拠（一人称「わたし」、です/ます基調、絵文字は3〜5個、丁寧な技術解説がベース）
- 数値: ベンチマーク・コスト・レイテンシ等の定量データを可能な限り含める（データがない場合は「未計測」と明記）
- トレードオフ: メリットだけでなくデメリット・向かないケースを必ず書く
- 前提レベル: 導入で対象読者の前提知識レベルを明示する（初心者向けの基礎説明は省略）
